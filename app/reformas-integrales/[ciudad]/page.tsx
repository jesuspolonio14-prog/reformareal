...
export async function generateMetadata({ params }: { params: Promise<{ ciudad: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Reformas Integrales en ${d.nombre} — Precio y Presupuesto 2026`,
    description: `Precio de una reforma integral en ${d.nombre}: desde ${fmt(d.integral.basica[0])} €/m². Compara hasta 3 presupuestos de reformistas verificados. Estimación gratuita en 2 minutos.`,
    alternates: { canonical: `${base}/reformas-integrales/${ciudad}` },
    openGraph: {
      title: `Reformas Integrales en ${d.nombre} — Precio 2026`,
      description: `Desde ${fmt(d.integral.basica[0])} €/m². Presupuestos de reformistas verificados en ${d.nombre}.`,
      url: `${base}/reformas-integrales/${ciudad}`,
    },
  }
}
...
export default async function Page({ params }: { params: Promise<{ ciudad: string }> }) {
  ...
  const base = 'https://reformareal.com'
  const url = `${base}/reformas-integrales/${ciudad}`
  ...
  const schemas = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: `Reformas integrales ${d.nombre}`, item: url },
      ],
    },
    {
      '@context': 'https://schema.org',
      ...
      mainEntity: [
        {
          '@type': 'Question',
          name: `¿Cuánto cuesta una reforma integral en ${d.nombre}?`,
          acceptedAnswer: { '@type': 'Answer', text: `El precio de una reforma integral en ${d.nombre} oscila entre ${fmt(d.integral.basica[0])} €/m² para acabados básicos y ${fmt(d.integral.premium[1])} €/m² para acabados premium. Para un piso de 80 m², el coste total suele estar entre ${fmt(d.integral.basica[0] * 80)} € y ${fmt(d.integral.premium[1] * 80)} €.` },
        },
        {
          '@type': 'Question',
          name: '¿Qué incluye una reforma integral?',
          acceptedAnswer: { '@type': 'Answer', text: 'Una reforma integral incluye demolición, albañilería y tabiquería, instalación eléctrica completa, fontanería y saneamiento, solados y alicatados, carpintería (puertas y armarios) y pintura y acabados finales.' },
        },
        {
          '@type': 'Question',
          name: `¿Cuánto tiempo tarda una reforma integral en ${d.nombre}?`,
          acceptedAnswer: { '@type': 'Answer', text: `El plazo habitual para una reforma integral en ${d.nombre} es de 8 a 16 semanas, dependiendo del tamaño de la vivienda y la complejidad de los trabajos.` },
        },
      ],
    },
  ]
  ...
  const faqs = [
    { q: `¿Cuánto cuesta una reforma integral en ${d.nombre}?`, a: `El precio de una reforma integral en ${d.nombre} oscila entre ${fmt(d.integral.basica[0])} €/m² para acabados básicos y ${fmt(d.integral.premium[1])} €/m² para acabados premium. Para un piso de 80 m², el coste total suele estar entre ${fmt(d.integral.basica[0] * 80)} € y ${fmt(d.integral.premium[1] * 80)} €, dependiendo de la calidad de los materiales y el estado previo de la vivienda.` },
    ...
    { q: `¿Cuánto tiempo tarda una reforma integral en ${d.nombre}?`, a: `El plazo habitual para una reforma integral en ${d.nombre} es de 8 a 16 semanas, dependiendo del tamaño de la vivienda y la complejidad de los trabajos. Reformas de pisos de menos de 60 m² pueden completarse en 6-8 semanas.` },
    ...
    { q: `¿Necesito licencia para reformar en ${d.nombre}?`, a: `Las obras que afectan a la estructura o cambian el uso de espacios requieren licencia de obras mayor. Las reformas integrales que solo modifican el interior sin tocar la estructura necesitan comunicación previa o licencia de obras menores. Tu reformista debe gestionar los permisos necesarios en ${d.nombre}.` },
  ]

  const calidades = [
    { calidad: 'Básica',    rango: `${fmt(d.integral.basica[0])} – ${fmt(d.integral.basica[1])} €/m²`,   ejemplo: `${fmt(d.integral.basica[0] * 80)} – ${fmt(d.integral.basica[1] * 80)} € para 80 m²`,   color: '#65A30D' },
    { calidad: 'Estándar',  rango: `${fmt(d.integral.estandar[0])} – ${fmt(d.integral.estandar[1])} €/m²`, ejemplo: `${fmt(d.integral.estandar[0] * 80)} – ${fmt(d.integral.estandar[1] * 80)} € para 80 m²`, color: '#C4531A' },
    { calidad: 'Premium',   rango: `${fmt(d.integral.premium[0])} – ${fmt(d.integral.premium[1])} €/m²`,  ejemplo: `${fmt(d.integral.premium[0] * 80)} – ${fmt(d.integral.premium[1] * 80)} € para 80 m²`,  color: '#CA8A04' },
  ]
    ...
    <main className="min-h-screen bg-[#F7F3EE] text-[#1C1208]">
      ...
      <section className="bg-[#1C1208] text-[#F7F3EE] py-20">
        <div className="max-w-5xl mx-auto px-5">
          <FadeIn>
            <h2 className="text-3xl md:text-4xl font-black mb-3">¿Cuánto cuesta una reforma integral en {d.nombre}?</h2>
            ...
            {calidades.map((c, i) => (
              <FadeIn key={c.calidad} delay={i * 80}>
                <div className="bg-[#2A1E10] rounded-2xl p-6 h-full">
                  ...
                  <p className="text-xs text-[#B5A090]">{c.ejemplo}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="py-20 max-w-5xl mx-auto px-5">
        <FadeIn>
          <h2 className="text-3xl font-black mb-3">Precio reforma integral por zona de {d.nombre}</h2>
          ...
          {d.zonas.map((z, i) => (
            <FadeIn key={z.zona} delay={i * 60}>
              <div className="bg-white border border-[#E8DFD8] rounded-2xl p-5">
                ...
                <p className="text-[#C4531A] font-black">{fmt(z.min)} – {fmt(z.max)} €/m²</p>
              </div>
            </FadeIn>
          ))}
        </div>
      ...
      <section className="bg-[#C4531A] py-20 text-center px-5">
        <h2 className="text-3xl font-black text-white mb-4">¿Cuánto cuesta tu reforma integral en {d.nombre}?</h2>
      ...
      <section className="py-20 max-w-3xl mx-auto px-5">
        <FadeIn>
          <h2 className="text-3xl font-black mb-10">Preguntas frecuentes sobre reformas integrales en {d.nombre}</h2>
        </FadeIn>
      ...
      <section className="bg-white py-12 border-t border-[#E8DFD8]">
        <div className="max-w-5xl mx-auto px-5">
          <p className="text-sm font-semibold text-[#6B5B4E] uppercase tracking-wide mb-4">Otros tipos de reforma en {d.nombre}</p>
